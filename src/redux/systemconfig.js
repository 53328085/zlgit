...
const handlermenu = (Meundata, id, homeMenuNO) => {
  ...
  let lang ='cn' // window.localStorage.getItem('i18nextLng')?.slice(0, 2)?.toLowerCase() == 'zh' ? 'cn' : 'en'
...
const initialState = {
  ...
  inverterSN: null, //光伏发电   并网柜跳转到逆变器页面
}
...
const system = createSlice({
  ...
  reducers: {
    ...
    getinverterSN(state, { payload }) {
      state.inverterSN = payload
    }
...
export const inverterSN = state => state.system.inverterSN
