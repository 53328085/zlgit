...
`;
export default forwardRef(function Index({ getLinePoint, projectId }, ref) {
  ...
  const getTreeData = async (roomId, projectId) => {
    const res = await DistributionRoomRuntime.GetLineTree({
      projectId,
      roomId,
    });
    if (res.success) {
      if (Array.isArray(res.data)) {
        ...
        let ids = dataList.current.map((d) => d.id);
        setSelectedKeys(ids);
        getLinePoint(ids);
        setstate({
          ...state,
