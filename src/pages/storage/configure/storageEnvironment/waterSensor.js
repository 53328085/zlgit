...
export default function Index(props) {
  ...
  const getFromHeader = () => {
    ...
    QueryWByPage(projectId, params).then(res => {
      if(res.success){
        if(res.data && res.data.length> 0){
  ...
  const onApplication = async () => {
    ...
    AddW(projectId, values).then(res => {
  ...
  const onAdd = async () => {
    ...
    if(state.modalTitle == '编辑水浸传感器'){
      let params = values
      params.id = state.selectId
      UpdateW(projectId, params).then(res => {
        let {success, data} = res
        if(success){
  ...
  const setMulti = item => {
    ...
    let params = {
      projectId, 
      areaId: addForm.getFieldValue('areaId'), 
      siteId: addForm.getFieldValue('siteId'), 
      pageNum: 1, 
      pageSize: 10000 
    }
    FindContainerList(projectId, addForm.getFieldValue('areaId'), addForm.getFieldValue('siteId')).then(res => {
  ...
  const onUpload = () => {
    ...
    BatchImportW(formData).then(res => {
      if(res.success){
  ...
  //设备类型
  useEffect(()=> {
    QueryCategoryUsed(projectId, 7).then(res => {
      if(res.success){
        if(res.data && res.data.length > 0){
          state.AddcategoryList=res.data
        }else{
          state.AddcategoryList=[]
          message.warning('当前项目不存在水浸传感器类型!')
        }
      }else{
        message.error(res.errMsg)
      }
  ...
  return (
    <Mainbox>
      ...
      <Custmodl  title="批量导入" mold="cust"   open={addModal} onOk={onUpload} onCancel={handleCancel} width={600}  >
      ...
      <Custmodl mold="cust" ref={addedit} title={state.modalTitle}    onOK={onAdd} width={544} custft={state.modalTitle == '新增水浸传感器'}>
        
        
          <CForm form={addForm} colon={false} labelCol={{ span: 7 }} labelAlign='left' requiredMark={false}>
            ...
            <Item name='sn' label='传感器编号' rules={[{ required: true, message: '请输入水浸传感器编号' }]} >
              <Input style={{ width: '320px' }} placeholder='请输入水浸传感器编号' disabled ={ state.modalTitle == '新增水浸传感器' ? false : true }></Input>
            </Item>
            <Item name='category' label='传感器型号' rules={[{ required: true, message: '请选择水浸传感器型号' }]} >
              <Select
                placeholder="请选择水浸传感器型号"
                size="middle"
                style={{ width: '320px' }}
              >
            ...
            <Item name='name' label='传感器名称' rules={[{ required: true, message: '请输入水浸传感器名称' }]} >
              <Input style={{ width: '320px' }} placeholder='请输入水浸传感器名称'></Input>
            </Item>
