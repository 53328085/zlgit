...
/*   const [tabs, setTabs]=useState([
    { key: '1', label: '分区档案' },
    { key: '2', label: '关联表具', disabled: true },
    { key: '3', label: '报警配置', disabled: true },
  ]) */
  const {search,pathname, state} = useLocation()
  const navigate = useNavigate()
  const id =  new URLSearchParams(search)?.get?.("id")  
  const  [tabs, title] = useMemo(()=> {
    return [[
      { key: '1', label: '分区档案' },
      { key: '2', label: '关联表具', disabled: !Boolean(id) },
      { key: '3', label: '报警配置', disabled: !Boolean(id) },
    ], id ? "编辑分区" : "新建分区"]
  },[id])
  const dataProps ={
    value,
