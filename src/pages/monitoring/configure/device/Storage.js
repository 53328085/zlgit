...
export default function Storage ({ deviceStyle, name }) {
  ...
  const editOk = () => {
    editform.validateFields().then(async () => {
      ...
      const resp = await UpdateStorageDevice(params)
      if (resp.success) {
  ...
  //确认删除
  const delOk = async () => {
    const { success, errMsg } = await DeleteStorageDevice({
      projectId,
  ...
  const addOk = async () => {
    return addform.validateFields().then(async () => {
      ...
      const res = await AddStorageDevice(params)
      if (res.success) {
  ...
  //获取电表列表
  const getQueryByPageStorageDevice = async (curpage = 0, pageSize = 0, id, like, customerType) => {
    try {
      setLoading(true)
      let params = {
        projectId,
  ...
  const ComProps = {
    ...
    page,
    exportExecel,
    getList: getQueryByPageStorageDevice,
    tb: tableLoadRef
  }
  ...
  return (
    <div>
      <Comp {...ComProps}>
        <Table columns={columns} pagination={page} paginationShow={true} dataSource={dataSource} loading={loading}
               ref={tableLoadRef} onChange={(page, pageSize) => {
          setPage(() => ({
            ...page
          }))
