...
export default function gateway({ deviceStyle }) {
    ...
    editform.validateFields().then(async () => {
      ...
      if (resp.success) {
        if (type == 'submit') {
          ...
          getQueryByPageHotWater376()
        } else {
          message.success("更新成功")
        }
        getQueryByPageHotWater(pageRef.current.current, pageRef.current.pageNum, compRef.current.selvalue, compRef.current.inpvalue, compRef.current.energyVal)
      } else {
  ...
  //获取热水表列表
  const getQueryByPageHotWater = async (curpage = 0, pageSize = 0, id, like, customerType) => {
    setLoading(true)
    let params = {
      projectId,
  ...
  //获取热水表列表
  const getQueryByPageHotWater376 = async () => {
    let params = {
      projectId,
      pageNum: 1,
  ...
  const onImportOk = async () => {
    ...
    const res = await ImportHotWater(formData)
    if (res.success) {
      if (res.data.success) {
  ...
  const ComProps = {
    ...
    page,
    exportExecel,
    getList: getQueryByPageHotWater,
    tb: tableLoadRef
  }
  ...
  const ImportProps = {
    modalImportRef,
    width: 560,
    link: '/deviceExcel/hotwater.xlsx',
    name: '热水表导入',
    uploadprops,
    onOk: onImportOk
  }
    ...
    <div>
      <Comp {...ComProps}>
        <Table columns={columns} pagination={page} paginationShow={true} dataSource={dataSource} loading={loading} ref={tableLoadRef} onChange={(page, pageSize) => {
          ...
          getQueryByPageHotWater(page.current, page.pageSize, compRef.current.selvalue, compRef.current.inpvalue, compRef.current.energyVal)
