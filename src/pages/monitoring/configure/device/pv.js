...
export default function gateway({ deviceStyle, name }) {
  ...
  //确认删除
  const delOk=async()=>{
    const {success,errMsg} = await DeletePV({
      projectId,
  ...
  //获取列表列表
  const getQueryByPagePv= async (curpage=0,pageSize=0,id, like,customerType) => {
  ...
  const onImportOk=async ()=>{
    ...
    const res = await ImportPV(formData)
     if(res.success) {
      if (res.data.success) {
    ...
    <div>
      <Comp {...ComProps}>
        <Table columns={columns} pagination={page} paginationShow={true} dataSource={dataSource} loading={loading}  ref={tableLoadRef} onChange={(page, pageSize) => {
          ...
          getQueryByPagePv(page.current, page.pageSize, compRef.current.selvalue, compRef.current.inpvalue, compRef.current.energyVal)
      ...
      <DeleteModal DelModalRef={DelModalRef} name="删除提示" content="是否确认删除光伏设备？" onOk={delOk}></DeleteModal>
      {EditModalComp}
      {/* <MyContext.Provider value={{ addopts, gatewaylist:gatewayRef.current, devicelist:deviceRef.current, alarmopts:alarmoptsRef.current, form: editform, deviceStyle,levelname }}>
        <EditModalForm {...EditModalFormProps}></EditModalForm>
      </MyContext.Provider> */}
