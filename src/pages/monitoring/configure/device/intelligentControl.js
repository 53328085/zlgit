...
export default function gateway({ deviceStyle }) {
    ...
    const editOk = () => {
        ...
        editform.validateFields().then(async () => {
            ...
            const resp = await UpdateSmartControl(params)
            if (resp.success) {
    ...
    //确认删除
    const delOk = async () => {
        const { success, errMsg } = await DeleteSmartControl({
            projectId,
    ...
    const addOk = async () => {
        return addform.validateFields().then(async () => {
            ...
            const res = await AddSmartControl(params)
            if (res.success) {
    ...
    const getQueryByPageElectric = async (curpage = 0, pageSize = 0, id, like, customerType) => {
        ...
        const resp = await QueryByPageSmartControl(params)
        setLoading(false)
        setPage({
            ...page,
    ...
    const onImportOk = async () => {
        ...
        const res = await ImportSmartControl(formData)
        if (res.success) {
            if (res.data.success) {
    ...
    return (
        <div>
            ...
            <DeleteModal DelModalRef={DelModalRef} name="删除提示" content="是否确认删除智能控制？" onOk={delOk}></DeleteModal>
            {EditModalComp}
