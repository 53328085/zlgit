  ...
  return (
    <Pagecount pd="0" bgcolor="transparent">
      <div className={style.cardList}>
        ...
        <Icard img={imgurl.screen} title={'直流屏'} value={statistics.sensorCount} key="screen" />

        <Icard img={imgurl.cupboard} title={'高压柜'} value={status.gatewayCount}
          isShow={true} on={'高压柜在线'} off={'高压柜离线'} per={'在线率'} onValue={status.gatewayOnlineCount}
          offValue={status.gatewayOfflineCount} perValue={status.gatewayOnlineRate} isRed={true} isGreen={true} isredE={false} after="%" key="gateway" />
        <Icard img={imgurl.screen} title={'出线柜'} value={statistics.sensorCount} key="chuan" />
        <Icard img={imgurl.screen} title={'温控仪'} value={statistics.sensorCount} key="chuan" />
