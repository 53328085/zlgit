...
export default function Index({formData}) {
  ...
  const [Ctitle,msg] = useMemo(()=> {
   let title =  ["新建线路控制方案" , "编辑方案", "新建路灯控制方案"][isadd]
 ...
 const title=<Title>
  <span>路灯线路控制方案列表</span>
  <Space size={16}>
            <CustButton   onClick={()=> onAdd()}>新建方案</CustButton>
            <CustButton  disabled={!strategyId}  onClick={()=> onBind()}>绑定线路</CustButton>
  ...
  return (
     <>
      <Titlelayout layout="flex" title={title}>
        ...
        <UserTable columns={columns} {...tableProps} onExport={onExport} rowKey={row => row.strategyId}  ref={tbref} rowSelection={rowSelection}  sheetName="线路方案"></UserTable>
