      ...
      <Mainwrap>
        <Titlelayout layout="flex" title={Ctitle} dr="column"  >
          ...
          <Radio.Group value={schemeId} onChange={onChange} style={{ marginTop: "16px" }}>
            ...
            renderItem={(item) => (
              <List.Item>
              <div className="listItem"><span>{item}</span><DeleteOutlined style={{color:"#909399", cursor: "pointer"}} onClick={()=>ondel(item)} /></div>   
              </List.Item>
            )}
            ></List>
        ...
        <div className="right">
          <Form form={form}  >
            <Space size={16}>
               <Form.Item label="报表名称"  name="reportName" rules={[
                {
                  ...
                  validator: (_, value) => {
                    if (!value) {
                      ...
                      let val = value.trim()
                      if (val.length > 20) {
                        return Promise.reject(new Error("报表名称长度不能超过20个字符"))
                      }
                    }
                  }
                }
               ...
               <Form.Item label="报表设备列名称" name="deviceName" rules={[{
                required:true,
                message: '请输入报表设备列名称',
                whitespace: true,
                validator: (_, value) => {
                  if (!value) {
                    ...
                    let val = value.trim()
                    if (val.length > 20) {
                      return Promise.reject(new Error("报表设备列名称长度不能超过20个字符"))
                    }
                  }
                }
               }
          ...
          <Titlelayout layout="flex" title={Dtitle} dr="column"> 
           ...
           <Bindcom ref={bindRef}></Bindcom>
             <CModal title="删除报表提示"  ref={delRef} width={512} mold="cust" type="warn" onOk={onOkDel} >
             操作不可撤销，是否确定删除  {reportName}？
                          </CModal>
      </Mainwrap>
    </Pagecount>
  );
