...
import {UserOutlined, PoweroffOutlined, ExclamationCircleFilled, PlusCircleOutlined, SettingOutlined, SearchOutlined, DesktopOutlined, ContactsOutlined} from '@ant-design/icons'
...
export default function Index() {
 ...
 const columns = [
    ...
    },
    {
      ...
      render: () => (
        <Space size={32}> 
            <CustBtn icon={<SettingOutlined style={{fontSize: '20px'}}/>}>项目配置</CustBtn>
            <CustBtn icon={<DesktopOutlined style={{fontSize: '20px'}} />} onClick={() => navigate('/index', {state: {headerKeys: '/index', index: true, title: '首页'}})}>进入项目</CustBtn>
        </Space>
      )
    ...
    <Mainbox>
       ...
       <div className='maincontent'>
          ...
          >
            ...
            <Item>
                <CustBtn icon={<PlusCircleOutlined  style={{color: '#fff', fontSize: '24px'}}  />}>新增项目</CustBtn>
            </Item>
            <Item name="projectName">              
                 <Input.Group compact>
                    <Input style={{ width: '412px' }} placeholder="请输入项目名称" />
                    <CutSerachBt width='98px' icon={<SearchOutlined style={{fontSize: '18px'}} onClick={submit} mgl="8px" />}> 查询</CutSerachBt>
                 </Input.Group>
            </Item>
            <Item>
            ...
            <Item name="valid">
                <Select placeholder="项目状态" style={{width: '200px'}} onChange={ submit}>
                    {options.map(o => (<Option value={o.value} key={nanoid()}>{o.label } </Option>))}
                </Select>
            </Item>
          ...
          <Table columns={columns} {...tableProps} rowClassName="rowclass" rowKey='id' bordered={true} pagination={{position: ['bottomCenter']}}> </Table>
       </div>
