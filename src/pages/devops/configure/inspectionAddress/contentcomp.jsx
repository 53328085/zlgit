...
export default forwardRef(({  getQueryPageDevice,areaId,addform, laptop }, ref) => {
    ...
    const columns = [
        ...
        { title: '检查项类别', dataIndex: 'type', align: "center",width: 180,render(text){
           return maparr.get(text)
        } },
   ...
   if(res.success){
    ...
    const onSearch = async (value, event) => {
        ...
        if(devicetype===0){
            ...
            if (!value) {
                filterarr=  copydataSource.filter(it=>{
                    return it.type === devicetype
                })
                setDataSource([...filterarr])
                return
            }else{
                filterarr = copydataSource.filter(it => {
                    return (it.name.includes(value))&&it.type === devicetype
                })
            }
