...
import { adaptation,selectOneLevelDefaultId } from "@redux/systemconfig";
import Subareas from "@com/useSerach/subareas"
import { isObject } from "@com/usehandler";
import {useQueryEnergyConfigedDevicesInfo} from "./api"
...
const getDatas=async()=>{
  try {
     let levelId = form.getFieldValue("sublevel")
     console.log(levelId)
    let params ={
      ...props.params,
      areaId:levelId
    }
    let {success, data,errMsg} = await useQueryEnergyConfigedDevicesInfo(params)
    if(success && isObject(data)){
       const {relations, noRelations} = data
        let selected = Array.isArray(relations) ? relations : [] 
        let unarr = Array.isArray(noRelations) ? noRelations : []       
        setSubData(selected);
        setSubCopy(selected);
  ...
  useEffect(() => {
    if(isObject(props.params) && Number.isInteger(parseInt(areaId))) { 
       getDatas()
  ...
  const handleSave = () => {
    if (clickSearch) {
      ...
      let areaId =  form.getFieldValue("sublevel")  
      
      console.log("保存", form.getFieldValue("sublevel"));
      props.saveValue({
        mainData,
        subData,
        unknownData,
                ...
                <div className="otherSubTable">
                  ...
                  <div className="searchInput">
                    ...
                    <Search
                      ...
                      onSearch={onSearchSub}
                    ></Search>
                   {props?.showarea && <Form layout="inline" form={form} onValuesChange={onValuesChange}>
                      <Subareas {...areaprops} />
                    </Form>}
                  </div>
                  <div className="mainContent">
