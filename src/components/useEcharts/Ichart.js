...
export default function Ichart(props = {}) {
  ...
  if (typechart == 2) {
    ...
    console.log(
      !Array.isArray(xAxis.data),
      Array.isArray(xAxis.data) && xAxis.data?.length === 0,
      f
    );
    if (
      !Array.isArray(xAxis.data) ||
  ...
  if (typechart == 3) {
    if (
      !Array.isArray(pieData.data) ||
      (Array.isArray(pieData.data) && pieData.data?.length === 0)
    ) {
      return <Cempty tip={info} />;
    }
  }
  if (typechart == 5) {
    if (
      !Array.isArray(custoption?.series) ||
      (Array.isArray(custoption?.series?.[0]?.data) &&
        custoption?.series?.[0]?.data?.length === 0)
    ) {
      return <Cempty tip={info} />;
    }
  }
